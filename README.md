...
3. "Metadata.csv" - csv file containing data unique to each recording needed for analysis. The following columns are defined:
...
4. "Analyze ISO Fatigue V3.r" - Script used to analyze an individual isotonic type contraction. Raw data files of isotonic contractions are indicated by a file name containing "Iso", e.g. "10_02_23_Muscle1_Iso20_After.txt
   Requires edits for metadata matching the individual trials being analyzed. MTU Length from Metadata.csv must be entered in line 6. Data were compiled in creation of Table 1.
